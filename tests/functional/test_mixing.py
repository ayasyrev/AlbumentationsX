...
def test_calculate_mosaic_center_point(
    ...
    center_xy = calculate_mosaic_center_point(
        grid_yx=grid_yx,
...
def test_calculate_cell_placements(
    ...
    placements = calculate_cell_placements(
        grid_yx=grid_yx,
        cell_shape=target_size,
        target_size=target_size,
        center_xy=center_xy,
...
def test_filter_valid_metadata_image_incompatible_channels(primary_data_rgb_mask_grayscale, item_gray_mask_grayscale):
    ...
    with pytest.warns(
        UserWarning,
        match="incompatibility in 'image'.*Item 'image' has 2 dimensions, but primary has 3",
    ):
...
def test_filter_valid_metadata_mask_incompatible_channels(
    primary_data_rgb_mask_single_channel,
    item_rgb_mask_multi_channel,
):
...
def test_filter_valid_metadata_mask_compatible_dims_channels(
    primary_data_rgb_mask_single_channel,
    item_rgb_mask_single_channel,
):
...
def test_filter_valid_metadata_mask_compatible_primary_has_item_missing(
    primary_data_rgb_mask_grayscale,
    item_rgb_no_mask,
):
...
def test_process_cell_geometry_identity(base_item_geom) -> None:
    ...
    processed = process_cell_geometry(
        item=item,
        cell_shape=(target_h, target_w),
...
def test_assemble_single_cell(processed_cell_data_1) -> None:
    ...
    # Test for image
    canvas_img = assemble_mosaic_from_processed_cells(
        processed_cells,
        target_shape_img,
        dtype,
        "image",
...
def test_preprocess_selected_mosaic_items_basic():
    ...
    assert sorted(res1.keys()) == sorted(
        ["image", "mask", "bboxes", "keypoints"],
    )  # Check keys regardless of order
    ...
    assert sorted(res2.keys()) == sorted(["image", "mask", "bboxes", "keypoints"])
